...
Specifically, TRT rewrites each model using the TRT Network Definition API, and utilizes custom plugins (such as fused multi-headed attention). This is essential for peak performance, but this level of manual intervention means the benefits of a more generic model export + inference runtime are diminished. FasterTransformer's INT8 compatible models have been rewritten in C++ in order to leverage the best performance from cuBLASLt non-standard interleaved data layout. This limits the extensibility of existing INT8 model implementations for novel architectures.
